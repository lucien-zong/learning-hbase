	...
	public void run() {
		...
		try {
			...
			try {
				...
				FilterList fl = new FilterList(expressionFilter,filter1,filter2);
				...
				/*
				 * //3、聚合 if (action.equalsIgnoreCase("rowcount")) { //
				 * 有表，有列解释器，有查询条件(scan：按列查询)，进行聚合操作 // There must be only one
				 * family. Long rowCount = aClient.rowCount(table,
				 * columnInterpreter, scan); System.out.println("rowKey" +
				 * "f:source_center" + "f:dest_center");
				 * System.out.println("The result of the rowCount is " +
				 * rowCount); } else if (action.equalsIgnoreCase("max")) { Long
				 * max = aClient.max(table, columnInterpreter, scan);
				 * System.out.println("The result of the max is " + max); } else
				 * if (action.equalsIgnoreCase("min")) { Long min =
				 * aClient.min(table, columnInterpreter, scan);
				 * System.out.println("The result of the min is " + min); } else
				 * if (action.equalsIgnoreCase("sum")) { Long sum =
				 * aClient.sum(table, columnInterpreter, scan);
				 * System.out.println("The result of the sum is " + sum); } else
				 * if (action.equalsIgnoreCase("std")) { Double std =
				 * aClient.std(table, columnInterpreter, scan);
				 * System.out.println("The result of the std is " + std); } else
				 * if (action.equalsIgnoreCase("median")) { Long median =
				 * aClient .median(table, columnInterpreter, scan);
				 * System.out.println("The result of the median is " + median);
				 * } else if (action.equalsIgnoreCase("avg")) { Double avg =
				 * aClient.avg(table, columnInterpreter, scan);
				 * System.out.println("The result of the avg is " + avg); } else
